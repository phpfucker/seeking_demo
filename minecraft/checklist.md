...
- [x] **5.2 繁殖条件を満たしたユニークペアの抽出**
    ...
    - [ ] **5.2.5 親の詳細特性情報の補完（5.1.3-5.1.5連携対応）**
        - [ ] 5.2.5.1 5.1.3-5.1.5完了を待機後、EntityInfoServiceから特性データを直接取得する方式に移行
        - [ ] 5.2.5.2 抽出されたペアの各親に対して、5.1拡張されたJava MODから取得した現在の詳細特性を補完する処理を実装
        - [ ] 5.2.5.3 `breeding_pairs_result.json`の出力形式を拡張し、現在世代の親詳細特性も含めて保存する
...
- [ ] **5.3 AI連携用JSONの生成と送信**
    - [ ] 5.3.1 **繁殖ペア情報の抽出と要約JSON生成**
        - [ ] 5.3.1.1 `breeding_pairs_result.json`から繁殖ペアの親情報（ID、DNA、特性）を抽出
        - [ ] 5.3.1.2 各ペアの親データから必要最小限の情報を抽出してChatGPT送信用JSONを生成
        ...
        - [ ] 5.3.1.3 複数ペアに対応した配列構造でのデータ整理
        - [ ] 5.3.1.4 距離が近いペアを優先した並び替え機能
    - [ ] 5.3.2 **利用可能エンティティ情報の準備**
        - [ ] 5.3.2.1 `models_skins_animations.json`から利用可能エンティティ名リストを抽出
        - [ ] 5.3.2.2 エンティティ名のみの配列を生成（詳細パス情報は除外してトークン節約）
            - **抽出する項目**：`name`のみ
            - **除外する項目**：`model`, `texture`, `animation`パス情報
            - **出力形式**：`["villager", "witch", "zombie", "pig", ...]`
        - [ ] 5.3.2.3 ChatGPT送信用データに統合
    - [ ] 5.3.3 **トークン制限管理システム**
        - [ ] 5.3.3.1 tiktoken-nodeライブラリのインストールと設定
        - [ ] 5.3.3.2 正確なトークン計算機能の実装（`tiktoken.encodingForModel("gpt-3.5-turbo")`使用）
        - [ ] 5.3.3.3 トークン制限値の設定（実際の送信上限：3,000トークン）
        - [ ] 5.3.3.4 動的ペア数調整機能（距離近い順でペア優先選択、上限超過時の後方ペア削除）
